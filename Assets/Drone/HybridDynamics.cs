...
public class HybridDynamics
{
    ...
    // Quad-copter dynamic constants
    public double l = 0.175;
    public double r = 0.12;
    public double CT = 4.8*Math.Pow(10, -3);
    public double CQ = 2.35*Math.Pow(10, -4);
    public double nB = 4;
    public double mB = 5.5*Math.Pow(10, -3);
    public double e = 4*Math.Pow(10, -3);
    public double Jm = 4.9*Math.Pow(10, -6);
    ...
    public HybridDynamics(
        ...
        kb = CT * rho * A * Math.Pow(r, 2);
        ktau = CQ * rho * A * Math.Pow(r, 3);
        Jl = 0.25 * nB * mB * Math.Pow(r - e, 2);
        J = Jm + Jl;
    }
    ...
    private double[] getLongitudinal()
    {
        ...
        double T = kb * (Math.Pow(U1, 2) + Math.Pow(U2, 2) + Math.Pow(U3, 2) + Math.Pow(U4, 2));
        double Ubar = U1 - U2 + U3 - U4;
        double [][] cp = new double[4][];
        cp[0] = new double[] {0};
        cp[1] = new double[] {T/m};
        cp[2] = new double[] {
            (Iz - Ix / Iy) * P * R + 
            (l * kb * (Math.Pow(U3, 2) - Math.Pow(U1, 2)))/Iy + 
            J * P * Ubar / Iy
        };
        cp[3] = new double[] {1};
    ...
    private double[] getLateral()
    {
        ...
        cp[1] = new double[] {
            (Iy - Iz / Ix) * Q * R + 
            (l * kb * (Math.Pow(U2, 2) - Math.Pow(U4, 2)))/Ix + 
            J * Q * Ubar / Ix
        };
        cp[2] = new double[] {
            (Ix - Iy / Iz) * P * Q + 
            (ktau * (Math.Pow(U1, 2) - Math.Pow(U2, 2) + Math.Pow(U3, 2) - Math.Pow(U4, 2)))/Iz 
        };
        cp[3] = new double[] {1};
