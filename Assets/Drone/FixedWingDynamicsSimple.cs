...
public class FixedWingDynamicsSimple
{
    ...
    public FixedWingDynamicsSimple(
    ...
    private double[] FixedWingDynamics_getForces()
    {
        ...
        double V_a = Math.Sqrt(Math.Pow(Ubar, 2) + Math.Pow(Vbar, 2) + Math.Pow(Wbar, 2)) + 0.0000000001;
        ...
        // Aerodynamic
        double CD_0 = 0;
        double CD_alpha = getCD_Basic(alpha);
        double CD_q = 0 * chord / (2 * V_a) * q;
        double CD_ele = getCD_Elevator(alpha, Ue);

        double CY_beta = -0.3073 * beta;
        double CY_p = getCy_RollRate(alpha) * b * p / (2 * V_a);

        double CL_0 = 0;
        double CL_alpha = getCL_Basic(alpha);
        double CL_q = 7.9520 * chord / (2 * V_a) * q;
        double CL_ele = getCL_Elevator(Ue);
    ...
    private double[] FixedWingDynamics_getMoments() {
        ...
        double Cl_ail = getCl_Aileron(Ua);
        double Cl_rud = 0.0000001 * Ur;
        
        double Cm_0 = 0;
        double Cm_alpha = getCm_Basic(alpha);
        double Cm_q = -16.58 * chord / (2 * V_a) * q;
        double Cm_ele = getCm_Elevator(Ue);

        double Cn_beta = 0.0709 * beta;
        double Cn_p = getCn_RollRate(alpha) * b / (2 * V_a) * p;
        double Cn_r = getCn_YawRate(alpha) * b / (2 * V_a) * r;
        double Cn_ail = getCn_Aileron(alpha, Ua);
        double Cn_rud = 0.0000001 * Ur;
    ...
    private double getCn_Aileron(double alpha, double Ua) {
        ...
        return cn_aileron_values[0,0];
        // Perform bilinear interpolation
        // ret = interp2(Ua_values, alpha_values, cn_aileron_values, Ua, alpha, "linear");
        // ret(isnan(ret))=100;
        // return ret;
    }
